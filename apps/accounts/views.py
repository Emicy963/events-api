...
def organizer_profile_view(request):
    ...
    if request.user.user_type not in ["organizer", "both"]:
        return Response(
            {"error": "User is not an organizer"}, status=status.HTTP_403_FORBIDDEN
        )
    ...
    try:
        ...
        return Response(
            {"error": "Organizer profile not found"}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    if request.method == "GET":
        ...
        serializer = OrganizerProfileSerializer(
            organizer_profile, data=request.data, partial=True
        )
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
